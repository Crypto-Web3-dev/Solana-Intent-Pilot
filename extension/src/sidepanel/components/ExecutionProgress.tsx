...
export function ExecutionProgress({ phase }: ExecutionProgressProps) {
  // Map internal phase to our UI steps
  ...
  const isFailed = phase === "failed" || phase === "blocked";
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '16px 0' }}>
      ...
      {STEPS.map((step, index) => {
        ...
        let color = "#E0E0E0"; // Pending
        if (isCompleted) color = "#4CAF50"; // Success
        if (isActive) color = "#2196F3"; // Active
        if (isError) color = "#F44336"; // Error
