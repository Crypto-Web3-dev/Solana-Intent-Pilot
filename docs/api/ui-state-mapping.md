...
- 当执行链进入显式降级路径时，UI 必须把“降级/未完全验证”与真正成功预览区分展示
...
- 显式降级预览若允许继续，必须附带明显提示，且视觉上不得等同于 verified success
- bundle 请求的 `awaiting-signature` 必须展示请求级预览，而不是仅展示部分动作就绪
...
- `当前预览基于降级路径生成，结果未完成完整验证`
- `实时模拟暂不可用，请不要将当前结果视为最终成功保障`
